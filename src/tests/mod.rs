...
mod integration_tests {
    ...
    fn test_system_dir_marking() {
        ...
        assert!(
            node_modules.is_gitignored,
            "node_modules should be marked as gitignored"
        );
    }
    ...
    fn test_gitignore_patterns() {
        ...
        assert!(
            gitignore.is_ignored(&root_path.join("error.log")),
            "*.log should be ignored"
        );
        assert!(
            !gitignore.is_ignored(&root_path.join("README.md")),
    ...
    fn test_show_system_directories() {
        ...
        let root =
            scan_directory_with_legacy_gitignore(&root_path, &gitignore, usize::MAX, Some(false))
                .unwrap();
        ...
        let root_with_system =
            scan_directory_with_legacy_gitignore(&root_path, &gitignore, usize::MAX, Some(true))
                .unwrap();
